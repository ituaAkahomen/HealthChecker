...
namespace AnnualHealthCheckJs.Attributes
{
    using Data;
    ...
    public class PINAttribute : RegularExpressionAttribute
    {
        ...
        public PINAttribute()
            : base($"(^\\d{{7}}$)")
            //: base($"(^\\d{{7}}$)|{string.Join('|', context.HMOs.Select(h => h.PinRegex))}")
        {
            ErrorMessage = "Incorrect ID Pattern";
        }
    }
    ...
    public class RegularExpressionAttribute : ValidationAttribute
    {
        ...
        public RegularExpressionAttribute(string pattern)
            : base(() => "RegexAttribute ValidationError")
        {
        ...
        private void SetupRegex()
        {
            if (this.Regex == null)
            {
                ...
                {
                    throw new InvalidOperationException("Empty Regex string");
                }
