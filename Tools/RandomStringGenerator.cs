...
namespace AnnualHealthCheckJs.Tools
{
    ...
    public class RandomStringGenerator
    {
                                        ...
                                        bool UseSpecialCharacters = true)
        {
            ...
            AlphaNumericCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ2346789".ToCharArray();
        ...
        /// <summary>
        /// True if we need to use alphanumeric characters
        /// </summary>
        public bool UseAlphaNumericCharacters
        {
            get
            {
                return m_UseAlphaNumericCharacters;
            }
            set
            {
                if (CurrentUpperCaseCharacters != null)
                    CurrentGeneralCharacters = CurrentGeneralCharacters.Except(CurrentAlphaNumericCharacters).ToArray();
                if (value)
                    CurrentGeneralCharacters = CurrentGeneralCharacters.Concat(CurrentAlphaNumericCharacters).ToArray();
                m_UseAlphaNumericCharacters = value;
            }
        }
        ...
        /// </summary>
        public char[] AlphaNumericCharacters
        {
            get
            {
                return CurrentAlphaNumericCharacters;
            }
            set
            {
                if (UseAlphaNumericCharacters)
                {
                    if (CurrentAlphaNumericCharacters != null)
                        CurrentGeneralCharacters = CurrentGeneralCharacters.Except(CurrentAlphaNumericCharacters).ToArray();
                    CurrentGeneralCharacters = CurrentGeneralCharacters.Concat(value).ToArray();
                }
                CurrentAlphaNumericCharacters = value;
            }
        }
        ...
        private string PatternDrivenAlgo(string Pattern)
        {
            ...
            foreach (char character in Pattern.ToCharArray())
            {
                ...
                switch (character)
                {
                    ...
                    case 'z':
                        {
                            newChar = GetRandomCharFromArray(CurrentAlphaNumericCharacters, Characters);
                            break;
                        }
                    default:
                        {
        ...
        private bool m_UseUpperCaseCharacters, m_UseLowerCaseCharacters, m_UseNumericCharacters, m_UseSpecialCharacters, m_UseAlphaNumericCharacters;
        private int m_MinUpperCaseCharacters, m_MinLowerCaseCharacters, m_MinNumericCharacters, m_MinSpecialCharacters, m_MinAlphaNumericCharacters;
        private bool PatternDriven;
        private char[] CurrentAlphaNumericCharacters;
